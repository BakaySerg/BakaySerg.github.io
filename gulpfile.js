...
gulp.task('server', function() {
	browserSync.init({
		...
		notify: true //Не отображать уведомления browserSync в браузере
	});
	gulp.watch(paths.watch.pug, ['html']); // Cлежение за Pug-файлами
	gulp.watch(paths.watch.scss, ['css'], {readDelay: 400}); // Cлежение за Scss-файлами
...
gulp.task('html', function() {
	return gulp
		.src(paths.dev.html.src) // Источник pug-файлов
...
gulp.task('css', function() {
	return gulp
		.src(paths.dev.css.src) // Источник scss-файла(ов)
		...
		.on('error', notify.onError({
			...
			title:"Sass error"
		})) // Преобразование scss-файлов в css, вывод уведомления в случае ошибки
...
gulp.task('js', function() {
	return gulp
		.src(paths.dev.js.src) //Источник js-файла(ов)
...
gulp.task('img', function() {
	return gulp
		.src(paths.img.src) // Источник файлов изображений
...
gulp.task('replace', function() {
	var htmlProd = gulp.src(paths.prod.html.src).pipe(gulp.dest(paths.prod.html.dest)); // Перенос html-файлов
	var cssProd = gulp.src(paths.prod.css.src).pipe(gulp.dest(paths.prod.css.dest)); // Перенос css-файлов
	var jsProd = gulp.src(paths.prod.js.src).pipe(gulp.dest(paths.prod.js.dest)); // Перенос js-файлов
	var fontsProd = gulp.src(paths.prod.fonts.src).pipe(gulp.dest(paths.prod.fonts.dest)); // Перенос шрифтов
	var otherProd = gulp.src(paths.prod.other.src).pipe(gulp.dest(paths.prod.other.dest)); // Перенос остальных файлов
});

// Сборка проекта. Последовательно выполняет "clean", "sprite", "img" и "replace"
gulp.task('build', function(callback) {
	runSequence('clean', 'img', 'replace');
});
